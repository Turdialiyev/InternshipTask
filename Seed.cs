    ...
    public static async Task InitializeUserAsync(IApplicationBuilder app)
    {
        ...
        foreach(var user in users)
        {
            ...
            if(result.Succeeded)
            {
                ...
                {
                    logger.LogInformation($"{user.UserName} role is added.");
                }
                else
                {
                    logger.LogInformation($"{user.UserName} role is not added.");
                }
            }
            else
            {
                logger.LogInformation($"{user.UserName} user is created");
            }
        }
    }
}